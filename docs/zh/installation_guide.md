...
2. 下载并应用Patch。
   ...
   ```bash
   git clone --branch dev_for_poc_26930 https://gitcode.com/boostkit/onnxruntime.git kp_onnxruntime
   cd onnxruntime
   git apply ../kp_onnxruntime/patches/001-boostsra-onnxruntime-1.0.0-graph-optimization.patch
   git apply ../kp_onnxruntime/patches/002-boostsra-onnxruntime-1.0.0-parallel-executor.patch
   ```

3. 添加KDNN库。

   ```bash
   cp -r ../kp_onnxruntime/kdnn ./onnxruntime/core/kdnn
   ```
...
4. 编译安装。
   ...
   ./build.sh --config Release --build_shared_lib --parallel \
     ...
     --allow_running_as_root --skip_tests --build_wheel \
     --cmake_extra_defines onnxruntime_USE_KDNN=ON onnxruntime_USE_SVE=ON
   ```

   其中：

   - `onnxruntime_USE_KDNN=ON`：编译并注册 KDNN 内部算子和 MHA 图融合。
   - `onnxruntime_USE_SVE=ON`：启用 ORT/目标平台所需的 SVE 构建配置。
   - contrib ops 必须保持启用；不能使用 `onnxruntime_DISABLE_CONTRIB_OPS=ON`。
