...
为保证您可以顺利安全地使用Kunpeng Onnxruntime特性，请确保所使用的环境信息在已验证环境范围内。

**硬件要求**

已验证的硬件环境如[**表 1** 硬件要求](#%E7%A1%AC%E4%BB%B6%E8%A6%81%E6%B1%82)所示。
...
已验证的操作系统如[**表 2** 操作系统](#%E6%93%8D%E4%BD%9C%E7%B3%BB%E7%BB%9F)所示。
...
已验证的软件依赖环境如[**表 3** 软件要求](#%E8%BD%AF%E4%BB%B6%E8%A6%81%E6%B1%82)所示。

**表 3** 软件要求

| 项目      | 版本      | 说明                                              | 下载地址        |
| ------- | ------- | ----------------------------------------------- | ----------- |
| Python  | 3.11.15 | Python是Onnxruntime的构建过程中的辅助工具，起到自动下载依赖、配置环境等作用。 | 通过Yum源方式安装。 |
| CMake   | 3.30.3  | CMake是Onnxruntime的构建工具，要求CMake版本为3.28.3及以上。     | 通过Yum源方式安装。 |
| GCC/G++ | 12.3.1  | GCC（GNU Compiler Collection）是一种编程语言编译器。         | 通过Yum源方式安装。 |

## 编译安装

1. 获取Onnxrutime开源代码。假设安装路径为“/path/to“。
