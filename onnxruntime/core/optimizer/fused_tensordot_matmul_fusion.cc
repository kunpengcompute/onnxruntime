...
static bool MatchDynamicFinalShape(const Graph& graph, const Node& final_reshape,
                                   const NodeArg& original_input,
                                   gsl::span<const int64_t> free_axes,
                                   int64_t rank, int64_t n) {
  const Node* shape_input = graph_utils::GetInputNode(final_reshape, 1);
  if (shape_input == nullptr) {
    return false;
  }

  if (graph_utils::IsSupportedOptypeVersionAndDomain(*shape_input, "Cast", {1, 6, 9, 13, 19, 21, 23, 24, 25})) {
    const auto* to_attr = graph_utils::GetNodeAttribute(*shape_input, "to");
    if (to_attr == nullptr || !to_attr->has_i() ||
        to_attr->i() != ONNX_NAMESPACE::TensorProto_DataType_INT64) {
      return false;
    }
    shape_input = graph_utils::GetInputNode(*shape_input, 0);
  }

  if (shape_input == nullptr ||
      !graph_utils::IsSupportedOptypeVersionAndDomain(*shape_input, "Concat", {1, 4, 11, 13})) {
    return false;
  }

  const auto* axis_attr = graph_utils::GetNodeAttribute(*shape_input, "axis");
  if (axis_attr == nullptr || !axis_attr->has_i() || axis_attr->i() != 0 ||
      shape_input->InputArgCount().front() != 2) {
    return false;
  }

  const Node* gather = graph_utils::GetInputNode(*shape_input, 0);
  if (gather == nullptr ||
      !graph_utils::IsSupportedOptypeVersionAndDomain(*gather, "Gather", {1, 11, 13})) {
    return false;
  }

  const auto* gather_axis_attr = graph_utils::GetNodeAttribute(*gather, "axis");
  if (gather_axis_attr != nullptr && gather_axis_attr->has_i() && gather_axis_attr->i() != 0) {
    return false;
  }

  InlinedVector<int64_t> gather_axes;
  if (gather->InputDefs().size() < 2 ||
      !optimizer_utils::AppendTensorFromInitializer(graph, *gather->InputDefs()[1], gather_axes, true) ||
      gather_axes.size() != free_axes.size()) {
    return false;
  }

  if (!NormalizeAxes(gather_axes, rank)) {
    return false;
  }

  for (size_t i = 0; i < free_axes.size(); ++i) {
    if (free_axes[i] != gather_axes[i]) {
      return false;
    }
  }
  ...
  const Node* shape = GetOptionalIntCastInputNode(*gather, 0);
  if (shape == nullptr || !IsDefaultShapeNode(*shape) ||
      shape->InputDefs().empty() || shape->InputDefs()[0] == nullptr ||
      shape->InputDefs()[0]->Name() != original_input.Name()) {
    return false;
  }

  InlinedVector<int64_t> n_values;
  if (!optimizer_utils::AppendTensorFromInitializer(graph, *shape_input->InputDefs()[1], n_values, true) ||
      n_values.size() != 1 ||
      n_values[0] != n) {
    return false;
  }
...
int FusedTensordotMatMulFusion::Fuse(Node& reshape, Graph& graph, const logging::Logger& logger) {
  ...
  for (const NodeIndex matmul_index : matmul_indices) {
    ...
    bool final_shape_is_dynamic = false;
    ...
    if (!final_shape_is_dynamic) {
      ...
      bool pinning_is_sound = true;
      for (const int64_t dim : expected_shape) {
        if (dim == -1) {
          ++num_dynamic_dims;
        } else if (dim <= 0) {
          pinning_is_sound = false;
        }
      }
      pinning_is_sound = pinning_is_sound && num_dynamic_dims == 1;
      ...
      for (size_t i = 0; i < expected_shape.size(); ++i) {
        ...
        } else if (actual != -1 && !(actual > 0 && pinning_is_sound)) {
          shape_matches = false;
          break;
        }
      }
